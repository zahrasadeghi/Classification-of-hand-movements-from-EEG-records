...
class convmodel(nn.Module):
    def __init__(self, out_classes, drop=0.5, d_linear=124):
        ...
        self.conv1 = nn.Conv1d(32, 64, kernel_size=3, padding=0, stride=1)
        self.conv2 = nn.Conv1d(64, 128, kernel_size=127)
        self.bn = nn.BatchNorm1d(64)
        self.pool = nn.MaxPool1d(2, stride=2)
        self.linear3 = nn.Linear(128, 124)
        self.linear4 = nn.Linear(124, 6)
        self.dropout1 = nn.Dropout(drop)
        self.dropout2 = nn.Dropout(drop)
        self.dropout3 = nn.Dropout(drop)

        self.convol1 = nn.Sequential(self.conv1, nn.ReLU(inplace=True), self.bn, \
                                  self.pool, self.dropout1)

        self.convol2 = nn.Sequential(self.conv2, nn.ReLU(inplace=True))

        self.dense = nn.Sequential(self.linear3, nn.ReLU(inplace=True), self.dropout2, \
                                   self.dropout3, self.linear4)
